...
| **E3** | B4 范围 | **10 列硬门禁**（日周各 5；不含 `ma_alignment` / `dde_alert` 软层）；其余 B2 + 人工抽检 |
...
### 1.3 B4 列（extract 14 列；硬门禁 10 列）
...
|----------|-------------------|----------------|------|
| `short_ma_regime` | `ma_alignment` | 均线形态 | TA MA5/MA10 十值枚举 |
| `daily_rev_ddx2_slope_reversal` | `dde_alert` | DDE警惕 | **2026-06-16**：TA 2-bar DDX2 拐点；不对齐 123 5-bar |
...
- 日线 5 列硬门禁 + 周线 5 列硬门禁（共 **10** 列数值门禁）
- 可选：`ma_alignment` / `w_ma_alignment`、`dde_alert` / `w_dde_alert` 写入 extract 供 Excel，**不进** golden 硬比
...
| 再触发 | 改 **10 列硬门禁**相关算法 / adj 大回填 / 周线修复后 | 无；跑 pytest `test_b4_gate` |
...
| **B4 周线未显式验收** | golden 若只存日线 5 列则漏一半硬门禁 | P0 | golden 必须 **10 列硬门禁** + `week_end` |
...
B4 golden：仅 `pytest tests/test_b4_gate*.py` 或 `python -m scripts.verify_b4_gate`（发布/改算法）；**必须比对 10 列硬门禁**。
...
- `tests/fixtures/b4_gate/`：`dates.txt`、`sample_500.csv`、**列映射（10 列硬门禁 + week_end）**、枚举表
...
- 5×500 diff vs 123 → extract **14 列**、硬比 **10 列** mismatch≈0（BSE DDE 日周均单独 bucket）
- 至少 1 个代表日为 **week_end**（验证周线 APPEND 路径）
- **未通过不得冻结 golden**

### 阶段 2 — 冻结 G2（闸门 2）

- `golden_{date}.csv`：`ts_code, trade_date, week_end` + **10 列**硬门禁字段
...
| 模块 | 文件/区域 | 阶段 |
|------|-----------|------|
| B4 回归（10 列硬门禁） | `scripts/diff_vs_123.py`, `verify_b4_gate`, `tests/fixtures/b4_gate/` | 0–2 |
...
1. 产品契约以 §1 为准（**含周线 5 列硬门禁 + 日线 5 列，extract 仍 14 列**），可与 v1.11「全历史 DWS 快照」**有意偏离**（§4.2）。
2. P3 顺序：**B4（10 列硬门禁）→ golden → 30min（日+周）→ 删 123**。
