...
backend/
  config.py              # 环境变量加载
  cli.py                 # CLI 入口（check/fetch/calc/export/query/status 等子命令）
  ...
  db/
    connection.py        # DuckDB 连接、自检、WAL checkpoint、temp/内存管理、孤儿清理
    schema.py            # 完整 DDL（26 表 + 16 视图 + 48 索引）
  fetch/
    client.py            # tushare API 封装（限流、指数退避重试）
    ods_daily.py         # 双模式拉取：date-batched + stock-batched
  ...
  etl/
    base.py              # EMA/SMA/线性回归/安全浮点/fingerprint/insert_dws_batch
    b4_macd.py           # B4 MACD zone/trend（123 10,20,7 日线 + resample-W 周线）
    b4_alerts.py         # MACD/DDE 警惕信号
    divergence_structure.py  # MACD/DDE Level 2 结构背离（通达信顶底结构）
    divergence_tradable.py   # L2 可交易背离消费层
    build_dim.py         # 维度表构建（stock/date/concept）
    build_dwd.py         # DWD 三层：rebuild_all_dwd() + rebuild_dwd_for_stale()
    dwd_weekly_sql.py    # dwd_weekly_quote 滚动周线 SQL
    calc_macd.py         # MACD(12,26,9)
    calc_ma.py           # MA5/MA10
    calc_kpattern.py     # 7种K线形态 + 强度评分
    calc_dde.py          # DDX/DDX2
    calc_volume.py       # 量能
    calc_price_position.py  # 价格位置
    orchestrator.py      # 编排器：run_etl + run_calc
    error_handler.py     # 5级错误分级
    progress.py          # 进度心跳
    calc_router.py       # calc 模式路由（SKIP/APPEND/FULL）
    recalc_spec.py       # 各指标重算窗口注册表
    vector/              # 向量化 batch 计算（macd/dde/volume）
  api/
    app.py               # FastAPI app
    router.py            # 5 端点
...
- **DuckDB temp_directory：** 默认 `data/tmp/`。首次 `get_connection(read_only=False)` 自动清理残留 `.tmp` 目录。`atexit` + `SIGTERM/SIGINT` handler 优雅关闭
- **磁盘门禁：** 实际阈值 `max(MIN_DISK_FREE_MB, DB_SIZE/3)`；temp 目录阈值 `max(threshold/2, 1024 MB)`
- **polyfit 向量化：** `base.weighted_window_slopes(y, window, decay)` 闭式 WLS（权重为 `w²`），替代逐 bar `np.polyfit`。等价性 `atol=1e-9` 由 golden-master 锁定
- **进程级共享限流：** `client._InterfaceRateLimiter` — 按接口名滑动窗口 + `threading.Lock`
- **交易日历优先本地：** `dim_date` 覆盖区间内本地查 `is_trade_day=1`，否则回退 API
- **多线程 calc：** `ThreadPoolExecutor` + `resolve_calc_workers()`（默认 `min(cpu-1, 8)`）。🚫 禁用 `multiprocessing.Pool`
- **周线计算器一律只采样真周末 bar：** JOIN `dim_date WHERE is_week_end=1`，禁止直接查滚动 bar
