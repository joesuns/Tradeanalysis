...
# ===== Excel 导出 =====
# 从数据库直接导出（不重算），文件自动命名为 analysis_{date}_gen{now}.xlsx
...
fetch（数据拉取层）
├── 不传 --ts-code → date-batched 全市场并行拉取（3线程）
...
├── 缺数据 → 自动补拉（策略选择器：股票数>交易日数→date-batched并行，否则stock-batched串行。增量per-target-stock粒度。熔断器：连续5次失败中止）
...
- **Date-batched per-stock 增量检测:** `_get_trading_days` 接受 `ts_codes` 参数，per-target-stock 粒度。
