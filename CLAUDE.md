...
- **Spec：** `docs/superpowers/specs/2026-05-31-stock-analysis-data-model.md` (v1.11)
...
# ===== calc state 回填（缺 state 导致永久 FULL 时一次性运维）=====
python -m backend.cli backfill-state              # 全市场，仅补缺 state
...
- **同日复跑幂等闸门:** 全市场同 `calc_date` 已成功 calc 且无 stale ODS → `run_calc` 秒级退出（`calc idempotent skip`）；`calc --force` / `run --force` 强制重算。
