...
- **同日复跑快路径（L0 pipeline_shortcut）：** fetch 0 diff + 无 stale DWD + 已有 prior calc → **跳过 DWD+calc**，仍 export。`run_rebuild_dwd.data_completeness.pipeline_shortcut=true`。**`run --force` 穿透 L0**（仍 fetch；DWD 仅 `changed∪stale` 窄重建，禁止全库 rebuild；calc 走 `run_calc(force=True)`）。L0 失败时日志 `pipeline L0 gate: skip_dwd_calc=false reason=...`（fetch_rows_written / spec_stale / stale_dwd / no_prior_calc）。验收：`tests/test_etl/test_column_narrow_equivalence.py`（Wave5 narrow vs full，`atol=1e-9`）。
