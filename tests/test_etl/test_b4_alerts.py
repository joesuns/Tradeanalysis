...
@pytest.mark.parametrize("macd_input,dde_input,expected", [
    ...
    (np.array([1.0, -1.0, 0.5]), np.array([0.0, 0.0, 1.0, 0.0, 5.0]), "downturn_reverse"),
    # Λ-shape / slope turns negative → bearish → upturn_reverse
    (np.array([1.0, 2.0, 1.0]), np.array([0.0, 0.0, -1.0, 0.0, -5.0]), "upturn_reverse"),
])
def test_cross_indicator_alert_enum_equivalence(macd_input, dde_input, expected):
    ...
    dde_alerts = compute_ddx2_slope_alerts(dde_input, window=2, eps=0.0)
